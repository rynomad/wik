...
var spinner = function spinner(before, after) {
  ...
  if (before.type == "paragraph") {
    ...
    if (before.text == undefined) {
      var preFinalPatchArray = [scrubUri, stampUri, operator.ObjectOperation.Create(["text"], ""), objectOperation]
    } else {
    ...
    console.log(diffList, before, after)
    for (var i = 0; i < diffList.length; i++) {
      if (diffList[i].kind == "N") {
        ...
        patches.push(operator.ObjectOperation.Update(diffList[i].path, operator.TextOperation.Delete(0, diffList[i].lhs)))
        patches.push(operator.ObjectOperation.Update(diffList[i].path, operator.TextOperation.Insert(0, diffList[i].rhs)))
      } else if (diffList[i].kind == "A") {
        if (diffList[i].item.kind == "E") {
        ...
        } else if (diffList[i].item.kind == "N") {
          patches.push(operator.ObjectOperation.Update(diffList[i].path, operator.ArrayOperation.Insert(diffList[i].index, diffList[i].item.rhs)))
  ...
  wikResolver(before.wik, function(wik){
    var index = require("./wikUpdater.js")
    console.log(wik)
    for (var i = 0; i < wik.story.length; i++) {
      if (wik.story[i] == parentUri) {
        ...
        console.log(before, after, compound)
